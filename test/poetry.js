...
import { get_random_uid, parse_uid, get_poetry_data_by_uid } from "../dist/index.js";
...
describe("test parse_uid", () => {
    ...
    it("check object property", () => {
        ...
        expect(paresd)
            .to.have.property("type_string")
            .to.be.oneOf(["tang", "song", "ci"]);
        expect(paresd)
            .to.have.property("collection")
            .to.be.oneOf(["全唐诗", "全宋诗", "全宋词"]);
        expect(paresd).to.have.property("poetry_time").to.oneOf(["唐", "宋"]);
    });
});

describe("get a random poetry", async () => {
    let data = await get_poetry_data_by_uid(get_random_uid());
    // await expect(data).to.have.property("extra");
    // expect(data).to.have.property("poetry_data");
    console.log(data);
});
